...
def main():
    ap=argparse.ArgumentParser(description='Diameter CER/CEA/DWR/DWA visibility test')
    sub=ap.add_subparsers(dest='mode',required=True)
    sv=sub.add_parser('server',help='Run Diameter server')
    sv.add_argument('--bind',required=True,help='Bind IP address')
    sv.add_argument('--port',type=int,default=3868,help='Listen port (default 3868)')
    sv.add_argument('--origin-host',default='diameter-partner.lab.invalid',help='Origin-Host AVP')
    sv.add_argument('--origin-realm',default='lab.invalid',help='Origin-Realm AVP')
    sv.add_argument('--timeout',type=int,default=10,help='Socket timeout in seconds')
    cl=sub.add_parser('client',help='Run Diameter client')
    cl.add_argument('--host',required=True,help='Remote host')
    cl.add_argument('--port',type=int,default=3868,help='Remote port (default 3868)')
    cl.add_argument('--bind',required=True,help='Local bind IP')
    cl.add_argument('--origin-host',default='diameter-home.lab.invalid',help='Origin-Host AVP')
    cl.add_argument('--origin-realm',default='lab.invalid',help='Origin-Realm AVP')
    cl.add_argument('--timeout',type=int,default=10,help='Socket timeout in seconds')
    cl.add_argument('--watchdogs',type=int,default=3,help='Number of DWR cycles')
    cl.add_argument('--interval',type=float,default=1.0,help='Interval between DWR cycles')
    a=ap.parse_args()
    run_server(a) if a.mode=='server' else run_client(a)
